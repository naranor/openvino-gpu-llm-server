#!/bin/bash
set -e
echo "=== OpenVINO GPU API Server: Phi-3-Medium-Instruct (INT8) ==="
if [ -f "/opt/intel/oneapi/setvars.sh" ]; then source /opt/intel/oneapi/setvars.sh; fi

MODEL_REPO="OpenVINO/Phi-3-medium-4k-instruct-int8-ov"
MODEL_DIR="models/Phi-3-medium-4k-instruct-int8-ov"

if [ ! -d "$MODEL_DIR" ]; then
    echo "Downloading Phi-3-Medium (14B) INT8..."
