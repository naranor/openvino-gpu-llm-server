#!/bin/bash
set -e
echo "=== OpenVINO GPU API Server: Dolly-v2-12B (INT8) ==="
if [ -f "/opt/intel/oneapi/setvars.sh" ]; then source /opt/intel/oneapi/setvars.sh; fi

MODEL_REPO="OpenVINO/dolly-v2-12b-int8-ov"
MODEL_DIR="models/dolly-v2-12b-int8-ov"

if [ ! -d "$MODEL_DIR" ]; then
    echo "Downloading Dolly-v2-12B INT8..."
