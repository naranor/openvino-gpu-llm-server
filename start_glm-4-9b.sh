#!/bin/bash
set -e
echo "=== OpenVINO GPU API Server: GLM-4-9B-Chat-1M (INT8) ==="
if [ -f "/opt/intel/oneapi/setvars.sh" ]; then source /opt/intel/oneapi/setvars.sh; fi

MODEL_REPO="AIFunOver/glm-4-9b-chat-1m-openvino-8bit"
MODEL_DIR="models/glm-4-9b-chat-1m-int8-ov"

if [ ! -d "$MODEL_DIR" ]; then
    echo "Downloading GLM-4-9B (1M Context variant)..."
