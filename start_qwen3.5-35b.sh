#!/bin/bash
set -e
echo "=== OpenVINO GPU API Server: Qwen 3.5 35B-A3B MoE (INT4) ==="

# 1. Environment
if [ -f "/opt/intel/oneapi/setvars.sh" ]; then source /opt/intel/oneapi/setvars.sh; fi

# 2. Model Configuration
MODEL_REPO="OpenVINO/Qwen3.5-35B-A3B-int4-ov"
MODEL_DIR="models/Qwen3.5-35B-A3B-int4-ov"

if [ ! -d "$MODEL_DIR" ]; then
    echo "Downloading Qwen 3.5 35B-A3B OpenVINO INT4 (~23GB)..."
...
# Run server with 32k context and VERY conservative batch for MoE stability
